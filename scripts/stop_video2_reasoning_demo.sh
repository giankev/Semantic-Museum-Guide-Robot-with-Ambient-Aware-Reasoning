...
label="$(docker inspect -f '{{index .Config.Labels "org.museum.video2"}}' "${CONTAINER}" 2>/dev/null || true)"
if [[ "${label}" == reasoning-demo ]]; then
  ...
  echo 'Reasoning demo stopped; logs retained. The museum_tiago fallback container is untouched.'
elif [[ -z "${label}" ]]; then
  echo 'No owned reasoning demo is running.'
else
