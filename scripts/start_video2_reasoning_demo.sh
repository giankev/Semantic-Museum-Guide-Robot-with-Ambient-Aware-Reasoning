#!/usr/bin/env bash
# Video 2: existing Actor stack plus deterministic semantic reasoning; no navigation injection.
set -euo pipefail
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
if (($#)); then
  echo 'Usage: scripts/start_video2_reasoning_demo.sh (one structured request; navigation remains prepared)'
  exit 2
fi
CONTAINER="museum_video2_reasoning_${UID}"
"${REPO_ROOT}/scripts/stop_video2_reasoning_demo.sh"
"${REPO_ROOT}/scripts/stop_video1_animated_demo.sh"
MODE=actor
GUI=True
OBSERVE=()
AUDIT=()
...
if docker container inspect "${CONTAINER}" >/dev/null 2>&1; then
  echo "${CONTAINER} already exists. Stop its demo with scripts/stop_video2_reasoning_demo.sh." >&2
  exit 1
fi

DOMAIN="${VIDEO2_ROS_DOMAIN_ID:-108}"
...
RUN_DIR="${REPO_ROOT}/log/video2_reasoning/${RUN_NAME}"
REMOTE_RUN="/root/exchange/log/video2_reasoning/${RUN_NAME}"
mkdir -p "${RUN_DIR}/gazebo" "${RUN_DIR}/ros"
echo "Starting semantic demo; startup logs: ${RUN_DIR}/startup.log"
exec 3>&1
...
cleanup_error() {
  ...
  if ((code != 0 && OWNED == 1)); then
    echo "Video 2 reasoning demo failed (exit ${code}); evidence retained in ${RUN_DIR}" >&2
    ...
    if ((RUNTIME_STARTED == 0)); then
      ...
      echo "Gazebo/RViz retained for inspection. Stop: ./scripts/stop_video2_reasoning_demo.sh" >&2
    fi
  fi
}
trap cleanup_error EXIT
trap 'exit 130' INT TERM
docker run --rm -d --init --name "${CONTAINER}" --label org.museum.video2=reasoning-demo \
...
import time
rclpy.init()
n=Node('video2_domain_preflight')
...
for executable in reasoning_node semantic_route_dispatcher; do
  docker exec -d "${CONTAINER}" bash -c "${SETUP} && exec ros2 run museum_assistant ${executable} --ros-args -p use_sim_time:=true >${REMOTE_RUN}/${executable}.log 2>&1"
done
echo 'Waiting for Gazebo, one Actor, Nav2 and semantic interfaces...' >&3
DEMO_STATUS=0
docker exec -i "${CONTAINER}" bash -c "${SETUP} && exec python3 /root/exchange/scripts/video2_reasoning_monitor.py --output ${REMOTE_RUN}" >&3 || DEMO_STATUS=$?
echo "Gazebo/RViz remain open. Evidence: ${RUN_DIR}/reasoning_summary.json" >&3
echo 'Stop: ./scripts/stop_video2_reasoning_demo.sh' >&3
exit "${DEMO_STATUS}"
